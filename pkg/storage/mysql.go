...
// NewMySqlCertificateRepository creates a new certificate repository backed by a MySQL database
func NewMySqlCertificateRepository(userName, userPass, dbName string) CertificateRepository {
...
func (repo *mySqlRepository) Open() error {
	connector, err := mysql.NewConnector(repo.cfg)
	if err != nil {
		return err
	}
	repo.db = sql.OpenDB(connector)
...
func (repo *mySqlRepository) StoreCertificate(cert *Certificate) error {
